  ...
  const handleOk = async () => {
    try {
      ...
      for (const r of valuePayload) {
        if (!r.code || !r.name) {
          message.warning('Mỗi giá trị thêm phải có đủ mã và tên (hoặc xóa dòng trống)')
          setSubmitting(false)
          return
        }
      }
      ...
      await attributesService.create({
        ...
        values: valuePayload.length > 0 ? valuePayload : undefined,
      })
      const msg =
        valuePayload.length > 0
          ? `Đã tạo thuộc tính và ${valuePayload.length} giá trị`
          : 'Đã tạo thuộc tính'
      message.success(msg)
      onSuccess()
      onClose()
      ...
      message.error('Không tạo được thuộc tính')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
        ...
        >
          <Input placeholder="VD: SIZE, COLOR" autoComplete="off" />
        </Form.Item>
        <Form.Item
          name="name"
          label="Tên"
        ...
        <Form.Item label="Giá trị (tuỳ chọn)">
