...
export type CreateOutboundDialogProps = {
  open: boolean
  onClose: () => void
  onSuccess: () => void
}

export function CreateOutboundDialog({ open, onClose, onSuccess }: CreateOutboundDialogProps) {
  ...
  useEffect(() => {
    if (!open) return
    form.resetFields()
    form.setFieldsValue({ documentNo: '', documentDate: '', reason: '', notes: '' })
    void loadWarehouses()
  ...
  const handleOk = async () => {
    try {
      ...
      message.error('Không tạo được phiếu xuất')
    } finally {
      setSubmitting(false)
    }
  }
