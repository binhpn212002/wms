...
- Trường: `code`, `name`, `active`, `sort_order` (tùy chọn).
...
| `sort_order` | int, nullable | Không | Thứ tự hiển thị trong UI; `null` hoặc quy ước “cuối danh sách” tùy team |
...
- Sắp xếp danh sách: `(sort_order NULLS LAST, name)` hoặc `(sort_order, code)` tùy quy ước UI.
...
- **Body**: `{ "code", "name", "sort_order?", "active?" }`.
- **201** + body entity tạo được.

### `PATCH /master-data/attributes/:id`

- **Body**: partial; cho phép đổi `code`, `name`, `sort_order`, `active` nếu không vi phạm unique và quy tắc tham chiếu.
